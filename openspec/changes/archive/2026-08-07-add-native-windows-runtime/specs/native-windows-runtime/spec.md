...
Define the first practical native Windows client runtime for the Dev Flow core controller and CLI while keeping platform-specific mechanisms narrow and leaving release integration for later changes.
...
This change SHALL NOT alter current persisted task fields, Schema identifiers, product version, workflow definitions, or replay rules solely to add the Windows host backend. It SHALL NOT introduce migration, translation, or cross-platform task-transfer code.
