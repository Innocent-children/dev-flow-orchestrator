...
Every manifest entry is keyed by `(repository_id, path)` and contains the change kind, before and after worktree kind/mode/content digest and Git worktree object ID, before and after index entry set, producer record and action, classification, acceptance-criterion IDs, and an agent-supplied purpose. Every bounded path observation also binds its entry in the snapshot's immutable `HEAD` tree when present. A clean tracked path first discovered after preflight uses that `HEAD`-tree mode and object ID as the immutable original stage-zero and worktree identity rather than using the already changed index or classifying the original as `missing`. The controller derives paths, change kinds, content, worktree-object, `HEAD`-tree and index identities and producer identity; the agent supplies only bounded classification, criterion mapping, and purpose. Source apply requires claims to cover exactly every changed manifest path. Contract revision reconciliation requires claims for every retained inherited entry and every exactly adopted drift entry so replacement-criterion mappings cannot be silently carried or dropped. Unknown, omitted, duplicate, cross-root, or contract-incompatible claims fail without a record.
...
The total-action ceiling is the exact sum of reachable fixed mutations under the effective contract, the three class ceilings, the product-bounded reserve for reachable unique waiver, disposition, persisted-reuse, and prerequisite-refresh subjects, and one non-cancelled Dossier finalization; it cannot exceed 256. Because each permitted review execution can produce a new bounded set of at most 64 findings, the initial conservative finding-disposition reserve is exactly `review_ceiling × 64`; a plan whose complete route then exceeds 256 fails before dispatch. The initial plan reserves one prerequisite-refresh subject for each conservative prerequisite-bearing obligation, fingerprinted with the dependent reservation and its complete conservative prerequisite-reservation set. Same-contract replacement inherits that subject before it validates the expanded graph, requires every new prerequisite to remain covered by the inherited set, and never recomputes the total from the replacement's current prerequisite edges. A governance subject is unique by contract, mutation kind, and criterion, assurance, finding, obligation, or prerequisite fingerprint as applicable, and may reserve and consume at most one total-action unit. A persisted reuse, waiver, disposition, or prerequisite refresh consumes one total-action unit but no verification, review, or rework unit. A read-only reuse derivation that appends no record consumes nothing.
