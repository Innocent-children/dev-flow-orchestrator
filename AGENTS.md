...
- 在本仓库执行 Python 相关的开发、测试、校验、文档生成或辅助脚本时，必须使用项目
  的 `uv` 环境，例如 `uv run python -m unittest ...`、`uv run pytest ...` 或
  `uv run python /absolute/path/to/script.py ...`。
- 即使被调用的脚本位于仓库外，只要它用于检查本项目或需要本项目的开发依赖，也必须
  通过本项目的 `uv` 环境执行。
- 不得直接使用系统或用户级的 `python`、`python3`、`pip` 或 `pip3` 运行上述命令或
  安装依赖。依赖缺失时应通过项目的 `uv` 配置处理，不得向系统或用户级 Python 环境
  安装包。
- 本节约束开发与校验环境，不改变插件运行时代码仅依赖 Python 标准库的要求。
