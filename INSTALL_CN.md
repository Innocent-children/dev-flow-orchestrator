...
本指南说明 `0.6.7` 版本的版本化 release artifact 生命周期。它无需克隆或保留本仓库，
...
Repair 重新运行与已安装版本匹配的 bootstrap。对于 `0.6.7`，使用第 3 节中完全相同
的命令，并设置 `VERSION=0.6.7`。只有 complete startup、receipt、ownership 与
