        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            _write_json(
                lifecycle / "installation.json",
                {
                    ...
                    "bin_dir": str(Path(temporary) / "bin space"),
                    "marketplace_file": str(Path(temporary) / ".agents" / "plugins" / "marketplace.json"),
                    "codex_home": str(Path(temporary) / "isolated codex home"),
    ...
    def test_uninstall_rejects_stable_dispatcher_or_state_drift_before_copy(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            installation = {
                ...
                "bin_dir": str(Path(temporary) / "bin"),
                "marketplace_file": str(Path(temporary) / "marketplace.json"),
                "codex_home": str(Path(temporary) / "isolated codex home"),
                "plugin_id": "dev-flow-orchestrator@personal",
            }
