...
Compatibility model 0.4.0 replaces fixed test-and-review loops with task-scoped adaptive
assurance. Every task owns an immutable preflight origin and a roll-forward
change manifest. The closed `dev-flow-assurance-policy/0.4.0` derives only the
...
The current Dev Flow release supports macOS with Git, Python 3.9–3.14, and Codex
plugin/Hook support:
...
Dev Flow 0.4.0 provides complete personal delivery over one exact canonical set of one to
...
aggregate Delivery Dossier 0.4.0. `dev-flow-workflow/0.4.0` definitions declare cancellation
...
stage. A custom workflow must be a valid `dev-flow-workflow/0.4.0` JSON or YAML document
...
The contract schema is `dev-flow-delivery-contract/0.4.0`. It contains exactly
...
contains the installed launcher, CLI, and exact 0.4.0 data directory. The Skill
obtains one `dev-flow-agent/0.4.0` projection with a `repository_set` summary,
...
`dev-flow-workflow/0.4.0` artifacts declare one workspace role:
...
`openspec-tasks/0.4.0` semantic normalizer, which ignores only checkbox state.

Verification uses the `dev-flow-verification-coverage/0.4.0` contract with an
...
terminal `dev-flow-delivery-dossier/0.4.0` body contains the effective contract,
...
- Task state lives outside every target repository. The installed Dev Flow 0.4.0 Hook uses
  `<PLUGIN_DATA>/0.4.0`; it protects the plugin data root from common direct
...
- Every cardinality uses `dev-flow-repository-set-snapshot/0.4.0`,
  `dev-flow-agent/0.4.0`, repository-scoped resources, structured member and
  integration verification, and Delivery Dossier 0.4.0. Each aggregate snapshot
  nests one `dev-flow-workspace-snapshot/0.4.0` member value per canonical member.

## Further documentation

- [INSTALL.md](INSTALL.md): installation, replacement, installed acceptance,
  and troubleshooting.
- [ARCHITECTURE.md](ARCHITECTURE.md): contracts, `dev-flow-workflow/0.4.0`, bindings,
