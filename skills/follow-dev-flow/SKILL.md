...
Resolve this `SKILL.md` to an absolute path. Treat its directory as the skill directory and its grandparent as the plugin root. Invoke `<plugin-root>/scripts/dev_flow.py` with Python 3. Do not assume the current working directory, search for a globally installed copy, or copy the controller into a target repository.

Read the Dev Flow bootstrap or active-task checkpoint injected by the plugin hook. Capture its absolute controller path and data directory, and verify that the controller path matches the one resolved from this skill. The injected data directory is required even before the first task exists. If that bootstrap context is absent, stop before `start` and report that the plugin hooks were not loaded or trusted; do not guess a fallback directory.

Run the controller's `--help` and the selected subcommand's `--help` before the first state-changing call. Treat help text as human-readable usage. Pass `--data-dir "<injected-data-dir>"` explicitly on every controller call, including `list`, `show`, and `start`; never assume a Bash or exec tool inherits the hook-only `PLUGIN_DATA` value. For every operational call, parse the single JSON stdout object; treat a nonzero exit, invalid JSON, or `ok: false` as a failed command and preserve stderr for diagnosis.
